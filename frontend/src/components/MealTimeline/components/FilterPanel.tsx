...
import { FilterOptions, UserPreferences, DayMeals } from "../types";
...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  // Memoized values for available features
  const {
    hasDiabeticMeals,
    availableCulturalPreferences,
    availableMealTypes,
  } = useMemo(() => {
    const culturalPrefs = new Set<string>();
    ...
    weekData.forEach((day) => {
      day.meals.forEach((meal) => {
        ...
        // Check for diabetes-friendly meals
        if (meal.diabetesFriendly !== undefined) {
          hasDiabetic = true;
        }

        // Collect cultural preferences from foods
        meal.foods.forEach((food) => {
          if (food.culturalOrigin) {
            food.culturalOrigin.forEach((origin) => culturalPrefs.add(origin));
          }
        });
      });
    });
    ...
    return {
      hasDiabeticMeals: hasDiabetic,
      availableCulturalPreferences: Array.from(culturalPrefs),
      availableMealTypes: Array.from(mealTypes),
    };
  }, [weekData]);
  ...
  return (
    <div className="p-4 space-y-6">
      {/* Section: Meal Types - Only show available meal types */}
      {availableMealTypes.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {MEAL_TYPES.filter(type => availableMealTypes.includes(type)).map(
              (type) => (
                <motion.label
                  key={type}
      ...
      <div>
        ...
        <div className="space-y-4">
          {/* Diabetes-Friendly Toggle - Only show if diabetic meals exist */}
          {hasDiabeticMeals && (
